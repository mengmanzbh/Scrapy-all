...
class SinaSpider(scrapy.Spider):
    name = 'sina'
    allowed_domains = ['live.sina.com.cn/zt/f/v/finance/globalnews1']
    start_urls = ['http://live.sina.com.cn/zt/f/v/finance/globalnews1/']

    def noticeDjangoUpdate():
        print("noticeDjangoUpdate")

    def parse(self, response):
        time = response.css('p.bd_i_time_c::text').extract_first()
        content = response.css('p.bd_i_txt_c::text').extract_first()
        ...
        if count == 0:
          ...
          db.sina.insert({"source":"新浪财经","time": time, "content": content,"count":totalCount+1})
          # print("---------------noticeDjangoUpdate--------------")
